...
st.header("Readiness & Recovery")

rd_df = get_readiness_df(token, start, end, sandbox)

if rd_df.empty:
    st.info("No readiness data available for the selected date range.")
    st.stop()
...
st.subheader("Readiness Contributors Over Time")
contributor_cols = [
...
fig.update_layout(
    yaxis_title="Score (0-100)", template=CHART_TEMPLATE,
    legend=dict(orientation="h", yanchor="bottom", y=1.02),
)
st.plotly_chart(fig, use_container_width=True)

st.subheader("Weakest Link Analysis")
...
if available_cols:
    ...
    fig = contributor_bar(
        ...
        "Average Contributor Scores (lower = weaker)",
    )
    st.plotly_chart(fig, use_container_width=True)

st.subheader("Contributor Heatmap")
...
if not contrib_data.empty:
    fig = px.imshow(
        ...
        color_continuous_scale="RdYlGn", template=CHART_TEMPLATE,
    )
    st.plotly_chart(fig, use_container_width=True)

st.subheader("Workout Recovery Advisor")
...
if not workouts_df.empty:
    ...
    if not workout_readiness.empty:
        st.dataframe(
            workout_readiness[["day", "activity", "intensity", "duration_min", "readiness_score", "next_day_readiness", "readiness_delta"]].dropna(subset=["readiness_score"]),
        ...
        if not by_intensity.empty:
            fig = go.Figure(go.Bar(
                x=by_intensity["intensity"], y=by_intensity["readiness_delta"],
                marker_color=[COLOR_GOOD if v >= 0 else COLOR_BAD for v in by_intensity["readiness_delta"]],
            ))
            fig.update_layout(title="Average Next-Day Readiness Change by Workout Intensity", yaxis_title="Readiness Delta", template=CHART_TEMPLATE)
            st.plotly_chart(fig, use_container_width=True)
    else:
